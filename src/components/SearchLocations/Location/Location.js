...
const Location = props => {
  ...
  let addRemoveFavHandlers =
    styleFavIcon.color === "blue" ? e => add(e) : e => remove(e);

  const favIcon =
    props.loggedIn && props.isFavourite ? (
      ...
      <a href="#" onClick={addRemoveFavHandlers}>
        <span>
          <FontAwesomeIcon style={{ color: "blue" }} icon={faStar} />
        </span>
      </a>
    ) : null;
  return (
    <div className={classes.Location}>
      <div
        className={classes.LocationImageInfo}
      ...
      >
        <img
          src="https://www.meteoblue.com/website/images/flags/mk.svg"
          style={{ height: "15px", width: "15px" }}
        />
        <p>{trimLocationName(props.place)}</p>
        <span>
          {" "}
          {props.longitude.toFixed(2)} N {props.latitude.toFixed(2)} E
        </span>
      </div>
      <div>{favIcon}</div>
    </div>
