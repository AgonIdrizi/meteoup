...
it("renders", () => {
  const { asFragment } = render(<Header location={location} current={current} />);
  expect(asFragment()).toMatchSnapshot();
})

it("inserts text in h1", () => {
  const { getByTestId, getByText } = render(<Header location={location} current={current} />);
  expect(getByTestId('h1tag')).toHaveTextContent('Weather in Tetovo')
})

it("displays temp", () => {
  const {container} = render(<Header location={location} current={current} />)
