...
class WeatherDataInDetail extends Component  {
    ...
    render() {
      ...
      const dayInText = moment(this.props.hourlyForecastData[this.props.dayId].dt_txt).format('dddd')
      const hours = this.props.hourlyForecastData.map(elem =>(<div key={elem.dt}> {moment(elem.dt_txt).format('HH-00')}</div>))
      ...
      const temp = this.props.hourlyForecastData.map(elem => <Degrees key={elem.dt} degree={elem.main.temp_max}/>)
      const feltTemp = this.props.hourlyForecastData.map(elem => <Degrees key={elem.dt} degree={elem.main.temp_min}/>)
      const windDirection = this.props.hourlyForecastData.map(elem => (<div key={elem.dt}>{this.windDegreesToWords(parseFloat(elem.wind.deg))}</div>))
    ...
    return (
      <div className={classes.WeatherDataInDetail}>
        <div className={classes.SelectedDay} ><span>{dayInText}</span></div>
          {hours}
